...
void test_sqlite3() {
    ...
    if (!db) {
        ...
        db = flexy::SQLite3::Create(
            dbname, flexy::SQLite3::READWRITE | flexy::SQLite3::CREATE);
        if (!db) {
...
#undef XX
        ...
        if (rt != SQLITE_OK) {
            FLEXY_LOG_ERROR(g_logger) << "create table error " << db->getErrno()
                                      << " - " << db->getErrStr();
            return;
        }
    }
    db->execStmt("insert into user (name, age) values (?, ?)", "stmt_1", 1);
    auto stmt = flexy::SQLite3Stmt::Create(
        db, "insert into user(name, age, create_time) values(?, ?, ?)");
    if (!stmt) {
        FLEXY_LOG_ERROR(g_logger) << "create statement error " << db->getErrno()
                                  << " - " << db->getErrStr();
    }
    int64_t now = time(0);
    for (int i = 2; i < 10; ++i) {
        ...
        if (stmt->execute() != SQLITE_OK) {
            FLEXY_LOG_ERROR(g_logger)
                << "execute statement error " << i << " " << db->getErrno()
                << " - " << db->getErrStr();
        }
        stmt->reset();
    }
...
void test_sqlite3_mgr() {
    ...
    sql_mgr.execStmt("test_0", "insert into user (name, age) values (?, ?)",
                     "Jhon", 19);
    auto res = sql_mgr.query("test_0", "select * from user");
    printData(res);
    sql_mgr.execute("test_0",
                    "update uer set name = 'Jsoa', age = 15 where id = 2");
    sleep(1);
