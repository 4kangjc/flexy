...
static auto g_mysql_dbs = Config::Lookup(
    "mysql.dbs", std::map<std::string, std::map<std::string, std::string>>(),
    "mysql dbs");
...
static MYSQL *mysql_init(std::map<std::string, std::string> &params,
    ...
    if (mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str(),
        ...
        FLEXY_LOG_ERROR(g_logger)
            << "mysql_real_connect(" << host << ", " << user << ", " << passwd
            << ", " << dbname << ", " << port
...
int64_t MySQL::getLastInsertId() { return mysql_insert_id(mysql_.get()); }
...
std::shared_ptr<MySQL> MySQL::getMySQL() { return nullptr; }

std::shared_ptr<MYSQL> MySQL::getRaw() { return mysql_; }
...
const char *MySQL::cmd() { return cmd_.c_str(); }
...
int MySQLRes::getDataCount() { return mysql_num_rows(data_.get()); }

int MySQLRes::getColumnCount() { return mysql_num_fields(data_.get()); }

int MySQLRes::getColumnBytes(int idx) { return curLength_[idx]; }
...
int64_t MySQLRes::getInt64(int idx) { return ::atoll(cur_[idx]); }

uint64_t MySQLRes::getUint64(int idx) { return getInt64(idx); }

float MySQLRes::getFloat(int idx) { return getDouble(idx); }

double MySQLRes::getDouble(int idx) { return ::atof(cur_[idx]); }
...
MySQLStmt::ptr MySQLStmt::Create(const MySQL::ptr &db,
...
int MySQLStmt::bind(int idx, int8_t value) { return bindInt8(idx, value); }

int MySQLStmt::bind(int idx, uint8_t value) { return bindUint8(idx, value); }

int MySQLStmt::bind(int idx, int16_t value) { return bindInt16(idx, value); }

int MySQLStmt::bind(int idx, uint16_t value) { return bindUint64(idx, value); }

int MySQLStmt::bind(int idx, int32_t value) { return bindInt32(idx, value); }

int MySQLStmt::bind(int idx, uint32_t value) { return bindUint32(idx, value); }

int MySQLStmt::bind(int idx, int64_t value) { return bindInt64(idx, value); }

int MySQLStmt::bind(int idx, uint64_t value) { return bindUint64(idx, value); }

int MySQLStmt::bind(int idx, float value) { return bindFloat(idx, value); }

int MySQLStmt::bind(int idx, double value) { return bindDouble(idx, value); }
...
int64_t MySQLStmt::getLastInsertId() { return mysql_stmt_insert_id(stmt_); }
...
int MySQLStmtRes::getColumnBytes(int idx) { return datas_[idx].length; }

int MySQLStmtRes::getColumnType(int idx) { return datas_[idx].type; }

std::string MySQLStmtRes::getColumnName(int idx) { return datas_[idx].name; }

const std::string &MySQLStmtRes::getColumnName(int idx) const {
    return datas_[idx].name;
}

bool MySQLStmtRes::isNull(int idx) { return datas_[idx].is_null; }

#define XX(type) return *(type *)datas_[idx].data
...
bool MySQLStmtRes::next() { return !mysql_stmt_fetch(stmt_->getRaw()); }
...
MySQLStmtRes::MySQLStmtRes(const std::shared_ptr<MySQLStmt> &stmt, int eno,
...
MySQLTransaction::ptr MySQLTransaction::Create(const MySQL::ptr &mysql,
                                               bool auto_commit) {
...
MySQLTransaction::ptr MySQLTransaction::Create(MySQL::ptr &&mysql,
    ...
    MySQLTransaction::ptr rt(
        new MySQLTransaction(std::move(mysql), auto_commit));
    if (rt->begin()) {
        return rt;
    }
    return nullptr;
...
bool MySQLTransaction::begin() { return execute("BEGIN") == 0; }
...
int MySQLTransaction::execute(const char *fmt, va_list ap) {
    if (isFinished_) {
        FLEXY_LOG_ERROR(g_logger)
            << "transaction is finished, format = " << fmt;
        return -1;
    }
...
std::shared_ptr<MySQL> MySQLTransaction::getMySQL() { return mysql_; }
    ...
    if (auto it = conns_.find(name); it != conns_.end()) {
        if (it->second.size() - count_[name] != 0) {
            ...
            if (!rt->isNeedCheck()) {
                ...
                return MySQL::ptr(
                    rt, [this, name](MySQL *m) { freeMySQL(name, m); });
            }
