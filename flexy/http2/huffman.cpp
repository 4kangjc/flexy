...
int EncodeString(const char* in, int in_len, std::string& out, int prefix) {
    ...
    out.resize(len);
    int rt = hf_string_encode(in, in_len, prefix, (unsigned char*)out.data(), &len);
    out.resize(len);
    return rt;
}
...
int DecodeString(const char* in, int in_len, std::string& out) {
	...
	int len = FLEXY_MAX_HUFFMAN_BUFF_LEN(in_len);
	out.resize(len);
	int rt = hf_string_decode(h_node, (unsigned char*)in, in_len, out.data(), len);
	hf_finish(h_node);
	out.resize(rt);
	return rt;
}
...
int EncodeLen(std::string_view in) {
	return EncodeLen(in.data(), in.size());
}
