...
/*
Length：
帧有效负载的长度表示为无符号的 24 位整数。除非接收方为 SETTINGS_MAX_FRAME_SIZE 设置了较大的值，否则不得发送大于2 ^ 14（16,384）的值。
帧头的 9 个八位字节不包含在此长度值中。
...
相当于 HTTP/1 里 Chunked 分块结束标志（“0\r\n\r\n”）。

R：
保留的 1 位字段。该位的语义未定义，发送时必须保持未设置 (0x0)，接收时必须忽略。

Stream Identifier：
流标识符 (参见 第 5.1.1 节)，表示为无符号 31 位整数。值 0x0 保留用于与整个连接相关联的帧，而不是单个流。
*/
...
public:
    ...
    virtual bool writeTo(const ByteArray::ptr& ba, const FrameHeader& header) const = 0;
    virtual bool readFrom(const ByteArray::ptr& ba, const FrameHeader& header) = 0;
};
