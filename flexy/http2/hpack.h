...
enum class IndexType {
    ...
    WITH_INDEXING_INDEXED_NAME =
        1,  // Name 在索引表(包括静态表和动态表)中，Value
            // 需要编码传递，并同时新增到动态表中
    WITH_INDEXING_NEW_NAME =
        2,  // Name 和 Value 都需要编码传递，并同时新增到动态表中
    WITHOUT_INDEXING_INDEXED_NAME =
        3,  // Name 在索引表(包括静态表和动态表)中，Value
            // 需要编码传递，并不新增到动态表中
    WITHOUT_INDEXING_NEW_NAME =
        4,  // Name 和 Value 需要编码传递，并不新增到动态表中
    NEVER_INDEXED_INDEXED_NAME =
        5,  // Name 在索引表(包括静态表和动态表)中，Value
            // 需要编码传递，并永远不新增到动态表中
    NEVER_INDEXED_NEW_NAME =
        6,  // Name 和 Value 需要编码传递，并永远不新增到动态表中
    ERROR = 7
};
...
public:
    ...
    static int WriteString(const ByteArray::ptr& ba, const std::string& str,
                           bool h);
