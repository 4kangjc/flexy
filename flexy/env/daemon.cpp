...
std::ostream& ProcessInfo::dump(std::ostream& os) const {
    os << "[ProcessInfo parent_id = " << parent_id << " main_id = " << main_id
       ...
       << " main_start_time = " << TimeToStr(main_start_time) << " restart_count = "
       << restart_count;
    return os;
}
...
static int real_start(int argc, char** argv, std::function
...
static int real_daemon(int argc, char** argv, std::function
    ...
    while (true) {
        ...
        if (pid == 0) {
            ...
            FLEXY_LOG_ERROR(g_logger) << "fork fail pid = " << pid
            << "errno = " << errno << " errstr = " << strerror(errno);
        } else {
            int status = 0;
            waitpid(pid, &status, 0);
            if (status) {
                FLEXY_LOG_ERROR(g_logger) << "child crash pid = " << pid
                << " status = " << status;
            } else {
