...
public:
    ...
    virtual ~Servlet() {}
    virtual int32_t handle(const HttpRequest::ptr& request, const HttpResponse::ptr& response, const SockStream::ptr& sesion) = 0;
...
public:
    using callback = std::function<int32_t
                    (const HttpRequest::ptr&, const HttpResponse::ptr&, const SockStream::ptr&)>;
    FuncionServlet(callback&& cb);
    FuncionServlet(const callback& cb);
    int32_t handle(const HttpRequest::ptr& request, const HttpResponse::ptr& response, const SockStream::ptr& session) override;
private:
    callback cb_;
};
