...
public:
    ...
    virtual int32_t onConnect(const HttpRequest::ptr& header,
                              const WSSession::ptr& session) = 0;
    virtual int32_t onClose(const HttpRequest::ptr& header,
                            const WSSession::ptr& session) = 0;
    virtual int32_t handle(const HttpRequest::ptr& header,
...
public:
    ...
    using on_connect_cb =
        std::function<int32_t(const HttpRequest::ptr&, const WSSession::ptr&)>;
    using on_close_cb =
        std::function<int32_t(const HttpRequest::ptr&, const WSSession::ptr&)>;
    using callback = std::function<int32_t(const HttpRequest::ptr&,
                                           const WSFrameMessage::ptr&,
                                           const WSSession::ptr&)>;

    template <typename T>
    using enable_connect_close_cb =
        std::enable_if_t<std::is_convertible_v<T, on_connect_cb>>;

    template <typename T>
    using enable_callback =
        std::enable_if_t<std::is_convertible_v<T, callback>>;
    // FunctionWSServlet(const callback& cb, const on_connect_cb& connect_cb =
    // nullptr, const on_close_cb& close_cb = nullptr);
    // FunctionWSServlet(callback&& cb, on_connect_cb&& connect_cb = nullptr,
    // on_close_cb&& close_cb = nullptr);

    template <class Callback, class Connect = on_connect_cb,
              class Close = on_close_cb, typename = enable_callback<Callback>,
              typename = enable_connect_close_cb<Connect>,
              typename = enable_connect_close_cb<Close>>
    FunctionWSServlet(Callback&& cb, Connect&& connect_cb = nullptr,
                      Close&& close_cb = nullptr)
...
public:
    ...
    template <class Callback, class Connect = FunctionWSServlet::on_connect_cb,
              class Close = FunctionWSServlet::on_close_cb,
    ...
    void addWSServlet(const std::string& uri, Callback&& cb,
                      Connect&& connect_cb = nullptr,
                      Close&& close_cb = nullptr) {
        ServletDispatch::addServlet(uri, std::make_shared<FunctionWSServlet>(
                                             std::forward<Callback>(cb),
                                             std::forward<Connect>(connect_cb),
    ...
    template <class Callback, class Connect = FunctionWSServlet::on_connect_cb,
              ...
              typename = FunctionWSServlet::enable_connect_close_cb<Connect>,
              typename = FunctionWSServlet::enable_connect_close_cb<Close>>
    void addGlobWSServlet(const std::string& uri, Callback&& cb,
                          Connect&& connect_cb = nullptr,
                          Close&& close_cb = nullptr) {
        ServletDispatch::addGlobServlet(
            uri,
            std::make_shared<FunctionWSServlet>(
